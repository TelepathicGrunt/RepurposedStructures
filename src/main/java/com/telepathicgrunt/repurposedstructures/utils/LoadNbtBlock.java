...
public class LoadNbtBlock extends Block {
    ...
    public static void instantiateNbtBlock(){
        Registry.register(Registry.BLOCK, new ResourceLocation(RepurposedStructures.MODID, "load_nbt_block"), LOAD_NBT_BLOCK);
        Registry.register(Registry.ITEM, new ResourceLocation(RepurposedStructures.MODID, "load_nbt_block"), LOAD_NBT_ITEM);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        // Finds and gets all ResourceLocations for pieces
        ...
        for(File file : files){
            if(file.getAbsolutePath().contains("crimson") && file.getAbsolutePath().contains("village")){
                ...
                ResourceLocations.add(new ResourceLocation(modifiedFileName));
            }
        }

        // Size of area we will need
        int rowCount = (int) Math.ceil(ResourceLocations.size());
        int columnCount = 1;
        ...
        for(int pieceIndex = 1; pieceIndex <= ResourceLocations.size(); pieceIndex++){
            ...
            if(be instanceof StructureBlockBlockEntity){
                ...
                structureBlockBlockEntity.setStructureName(ResourceLocations.get(pieceIndex-1)); // set ResourceLocation
                ...
                structureBlockBlockEntity.setStructureName(new ResourceLocation(ResourceLocations.get(pieceIndex-1).toString().replace("village/", ""))); // set ResourceLocation
