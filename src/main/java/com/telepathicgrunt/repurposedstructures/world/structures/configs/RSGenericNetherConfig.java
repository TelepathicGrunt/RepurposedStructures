...
public class RSGenericNetherConfig extends RSGenericConfig {
    ...
    public static final Codec<RSGenericNetherConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.INT.fieldOf("set_fixed_y_spawn").orElse(Integer.MIN_VALUE).forGetter(config -> config.setFixedYSpawn),
            Codec.INT.fieldOf("center_y_offset").orElse(0).forGetter(config -> config.centerYOffset),
            Codec.BOOL.fieldOf("do_not_use_heightmap").forGetter(config -> config.doNotUseHeightmap),
            Codec.BOOL.fieldOf("cannot_spawn_in_liquid").forGetter(config -> config.cannotSpawnInLiquid),
            ...
            Codec.BOOL.fieldOf("search_for_highest_land").forGetter(config -> config.highestLandSearch),
