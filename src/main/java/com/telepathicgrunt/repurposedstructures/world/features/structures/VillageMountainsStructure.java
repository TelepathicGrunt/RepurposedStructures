...
public class VillageMountainsStructure extends Structure<NoFeatureConfig>
{
    /**
     * --------------------------------------------------------------------------
     * |									|
     ...
     */
    public VillageMountainsStructure(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
	super(config);
    }
	...
	int maxDistance = RepurposedStructures.RSVillagesConfig.mountainsVillageSpawnrate.get();
	...
	((SharedSeedRandom) random).setLargeFeatureSeedWithSalt(chunkGenerator.getSeed(), targetChunkX, targetChunkZ, 14983238);
	...
	return RepurposedStructures.MODID + ":village_mountains";
    }
	...
	public void init(ChunkGenerator<?> generator, TemplateManager templateManager, int chunkX, int chunkZ, Biome biomeIn) {
	    ...
	    VillagePieces.addPieces(generator, templateManager, blockpos, this.components, this.rand, new VillageConfig(RepurposedStructures.MODID + ":village/mountains/town_centers", 6));
