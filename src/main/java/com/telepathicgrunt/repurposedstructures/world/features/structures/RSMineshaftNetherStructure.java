...
public class RSMineshaftNetherStructure extends Structure<NoFeatureConfig>
{
	...
	if (random.nextDouble() < (RepurposedStructures.RSMineshaftsConfig.netherMineshaftSpawnrate.get() / 10000D)) {
	...
	public void init(ChunkGenerator<?> generator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biomeIn) {
	    RSMineshaftPieces.Room structuremineshaftpiecesua$room = new RSMineshaftPieces.Room(0, this.rand, (chunkX << 4) + 2, (chunkZ << 4) + 2, RSMineshaftPieces.Type.NETHER);
	    ...
	    int minimum = RepurposedStructures.RSMineshaftsConfig.netherMineshaftMinHeight.get();
	    int maximum = Math.max(RepurposedStructures.RSMineshaftsConfig.netherMineshaftMaxHeight.get(), minimum)+1;
