...
public class VillageDarkForestStructure extends Structure<NoFeatureConfig>
{
    /**
     * --------------------------------------------------------------------------
     * |									|
     ...
     */
    public VillageDarkForestStructure(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
	super(config);
    }
	...
	int maxDistance = RepurposedStructures.RSVillagesConfig.darkForestVillageSpawnrate.get();
	...
	((SharedSeedRandom) random).setLargeFeatureSeedWithSalt(chunkGenerator.getSeed(), targetChunkX, targetChunkZ, 14983235);
	...
	return RepurposedStructures.MODID + ":village_dark_forest";
    }
	...
	public void init(ChunkGenerator<?> generator, TemplateManager templateManager, int chunkX, int chunkZ, Biome biomeIn) {
	    ...
	    VillagePieces.addPieces(generator, templateManager, blockpos, this.components, this.rand, new VillageConfig(RepurposedStructures.MODID + ":village/dark_forest/town_centers", 6));
