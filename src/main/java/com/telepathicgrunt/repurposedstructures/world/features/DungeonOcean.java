...
public class DungeonOcean extends Feature<NoFeatureConfig>
{
		...
		for (int j4 = 0; j4 < 3; ++j4) {
			...
			if (j3 == 1) {
			    world.setBlockState(blockpos$Mutable, 
				    StructurePiece.correctFacing(world, blockpos$Mutable, 
	...
	if (roll < 96) {
		...
		String biomeName = "";
		if (biome.getRegistryName() != null) biomeName = biome.getRegistryName().getPath();

		float biomeTemp = biome.getDefaultTemperature();

		if (biomeTemp < 0.0 || BiomeDictionary.getTypes(biome).contains(Type.SNOWY) || biomeName.contains("frozen") || biomeName.contains("snow") || biomeName.contains("ice")) {
		    return EntityType.SQUID;
		}

		// deliberately skip 0.5 temp as all vanilla oceans are 0.5 and we are checking for vanilla oceans temp by name
		else if (biomeTemp < 0.5 || BiomeDictionary.getTypes(biome).contains(Type.COLD) || biomeName.contains("cold")) {
		...
		else if (biomeTemp >= 0.9 || BiomeDictionary.getTypes(biome).contains(Type.LUSH) || biomeName.contains("lukewarm")) {
		    return EntityType.PUFFERFISH;
		}
		else if (biomeTemp >= 1.5 || BiomeDictionary.getTypes(biome).contains(Type.HOT) || biomeName.contains("warm") || biomeName.contains("hot") || biomeName.contains("tropic")) {
