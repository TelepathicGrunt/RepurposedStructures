...
public abstract class WellAbstract extends Feature<NoFeatureConfig>
{
    ...
    public WellAbstract(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
	super(config);
    }


    protected Template generateTemplate(ResourceLocation templateRL, IWorld world, Random random, BlockPos position) {

	// cache to save time and speed
	if (templatemanager == null) templatemanager = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager();
	...
	Template template = templatemanager.getTemplate(templateRL);
	...
	template.addBlocksToWorld(world, position.add(offset), placementsettings);
	return template;
    }
     ...
     */
    protected static void addBells(String function, BlockPos position, IWorld world, Random random, Collection<Block> allOreBlocks) {
