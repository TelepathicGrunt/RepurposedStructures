...
public class SkeletonHorseman extends Feature<GenericMobConfig> {
    ...
    public boolean place(ISeedReader world, ChunkGenerator chunkGenerator, Random random, BlockPos position, GenericMobConfig config) {
        ...
        SkeletonHorseEntity skeletonHorseEntity = EntityType.SKELETON_HORSE.create(world.getLevel());
        skeletonHorseEntity.setPersistenceRequired();
        skeletonHorseEntity.absMoveTo(
                (double)position.getX() + 0.5D,
                position.getY(),
                (double)position.getZ() + 0.5D,
                0.0F,
                0.0F);
        skeletonHorseEntity.finalizeSpawn(world, world.getCurrentDifficultyAt(position), SpawnReason.STRUCTURE, null, null);
        ...
        if(config.heldItem != null){
            ...
            skeletonEntity.setItemInHand(Hand.MAIN_HAND, GeneralUtils.enchantRandomly(random, heldItem, 0.1F));
        ...
        world.addFreshEntityWithPassengers(skeletonHorseEntity);
        return true;
    }
}