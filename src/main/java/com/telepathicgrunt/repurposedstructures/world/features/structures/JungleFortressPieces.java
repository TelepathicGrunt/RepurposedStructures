...
public class JungleFortressPieces
{
	...
	public boolean create(IWorld world, ChunkGenerator<?> p_225577_2_, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos p_74875_4_) {
	    ...
	    if (RepurposedStructures.RSConfig.lootChestsJF.get() && random.nextInt(9) == 0 && structureBoundingBoxIn.isVecInside(new BlockPos(this.getXWithOffset(3, 3), this.getYWithOffset(2), this.getZWithOffset(3, 3)))) {
	...
	public boolean create(IWorld world, ChunkGenerator<?> p_225577_2_, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos p_74875_4_) {
	    ...
	    if (RepurposedStructures.RSConfig.lootChestsJF.get() && random.nextInt(9) == 0 && structureBoundingBoxIn.isVecInside(new BlockPos(this.getXWithOffset(1, 3), this.getYWithOffset(2), this.getZWithOffset(1, 3)))) {
	...
	public boolean create(IWorld world, ChunkGenerator<?> p_225577_2_, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos chunkPos) {
	    ...
	    if (RepurposedStructures.RSConfig.lootChestsJF.get()) {
	...
	protected BlockState getStoneVariantBlockState(BlockState blockstateIn, Random rand) {
		...
		float silverfishThreshold = (float) (RepurposedStructures.RSConfig.silverfishSpawnrateJF.get() / 100);
		...
		if (!allSoilBlocks.isEmpty()) soilBlock = ((Block) allSoilBlocks.toArray()[rand.nextInt(allSoilBlocks.size())]).getDefaultState();

		if (soilBlock.has(BlockStateProperties.MOISTURE_0_7)) soilBlock = soilBlock.with(BlockStateProperties.MOISTURE_0_7, rand.nextInt(8));
		...
		if (!allPlantBlocks.isEmpty() && chance < 0.4f) {
		    ...
		    else if (plantBlock.has(BlockStateProperties.AGE_0_1)) plantBlock = plantBlock.with(BlockStateProperties.AGE_0_1, rand.nextInt(2));
	...
	public boolean create(IWorld world, ChunkGenerator<?> p_225577_2_, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos chunkPos) {
		...
		if (structureBoundingBoxIn.isVecInside(blockpos)) {
			...
			if (RepurposedStructures.RSConfig.allowSilverfishSpawnerJF.get()) {
