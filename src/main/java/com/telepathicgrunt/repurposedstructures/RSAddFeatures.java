...
public class RSAddFeatures
{
    ...
    private static boolean serializeAndCompareFeature(ConfiguredFeature<?, ?> feature1, ConfiguredFeature<?, ?> feature2) {
		try {
	//	    Map<DynamicOps<Tag>, DynamicOps<Tag>> feature1Map = feature1.CODEC.serialize(NbtOps.INSTANCE).getMapValues().get();
	//	    Map<DynamicOps<Tag>, DynamicOps<Tag>> feature2Map = feature2.serialize(NbtOps.INSTANCE).getMapValues().get();
	//
